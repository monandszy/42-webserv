...
int poller::init_epoll(std::map<int, Server>& sockets) {
...
void poller::loop_epoll(int epoll_fd, std::map<int, Server>& sockets) {
  const int limit = 10;
  ...
  while (true) {
    ...
    for (int i = 0; i < num_ready; i++) {
      ...
      if (sockets.find(client_fd) != sockets.end()) {
        ...
        request_handler::process_request(client_fd);
        close(client_fd);
      }
    }
  }
